...
function ImportRow({ lead, idx, onRemove, onGenerate, onDeleteLead, generatingId, imported }) {
    ...
    <tr className="border-t border-ink-600 hover:bg-ink-700/30">
      ...
      <td className="px-3 py-2.5">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {lead._lead_id ? (
            <button onClick={() => onDeleteLead?.(lead._lead_id)} title="Remove from list"
              className="text-ash/40 hover:text-danger transition-colors">
              <Trash2 size={13} />
            </button>
          ) : !imported ? (
            <button onClick={() => onRemove(idx)} className="text-ash/40 hover:text-danger transition-colors">
              <Trash2 size={13} />
            </button>
          ) : null}
        </div>
...
export default function LeadFinder() {
  ...
  async function handleDeleteLead(leadId) {
    if (!leadId) return;
    if (!confirm("Remove this lead from your list? If a website was generated or an email was sent, the demo link and emails are kept.")) return;
    try {
      await api.delete(`/api/leads/${leadId}`);
      setResults((prev) => prev.filter((l) => l.id !== leadId));
      setImportLeads((prev) => prev.filter((l) => l._lead_id !== leadId));
    } catch (e) {
      setError(e.response?.data?.detail || "Could not delete lead");
    }
  }
  ...
  return (
    <DashboardLayout>
      ...
      <div className="card mb-8">
        <button onClick={() => setAddOpen((v) => !v)}
          ...
          <span className="flex items-center gap-2 font-medium text-sm">
            <UserPlus size={16} /> Add leads manually
            {importLeads.length > 0 && (
              <span className="ml-2 bg-signal/20 text-signal text-xs px-2 py-0.5 rounded-full">{importLeads.length}</span>
            )}
          </span>
              ...
              <div className="border-t border-ink-600 px-6 pb-6">
                <div className="flex gap-1 mt-4 mb-5 bg-ink-700 p-1 rounded-lg w-fit">
                  <button onClick={() => setAddMode("manual")}
                    className={`px-4 py-1.5 rounded-md text-xs font-medium transition-colors ${addMode === "manual" ? "bg-signal text-ink" : "text-ash hover:text-ash-light"}`}>
                    Type it in
                  </button>
                  <button onClick={() => setAddMode("file")}
                    className={`px-4 py-1.5 rounded-md text-xs font-medium transition-colors ${addMode === "file" ? "bg-signal text-ink" : "text-ash hover:text-ash-light"}`}>
                    Upload file
                  </button>
                </div>
                ...
                {addMode === "manual" && (
                  <form onSubmit={handleAddManual} className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                    <div className="col-span-2 md:col-span-1">
                      ...
                      <input className="input-field w-full" placeholder="e.g. Joe's Barbershop" value={manualForm.name} required
                    ...
                    <div>
                      <label className="text-xs text-ash block mb-1">City</label>
                      <input className="input-field w-full" value={manualForm.city}
                        onChange={(e) => setManualForm({ ...manualForm, city: e.target.value })} />
                    </div>
                    <div>
                      <label className="text-xs text-ash block mb-1">Phone</label>
                      <input className="input-field w-full" value={manualForm.phone}
                        onChange={(e) => setManualForm({ ...manualForm, phone: e.target.value })} />
                    </div>
                    <div>
                      <label className="text-xs text-ash block mb-1">Email</label>
                      <input className="input-field w-full" type="email" value={manualForm.email}
                ...
                {addMode === "file" && (
                  <div className="mb-4">
                    <div
                      ...
                      className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors ${dragging ? "border-signal bg-signal/5" : "border-ink-500 hover:border-signal/50"}`}>
                ...
                {importError && <p className="text-danger text-xs mb-3">{importError}</p>}

                {importLeads.length > 0 && (
                  <>
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-ash text-xs">{importLeads.length} leads in list</p>
                      <div className="flex items-center gap-3">
                        {!importDone && unimported > 0 && (
                          <button onClick={handleImport} disabled={importing} className="inline-flex items-center gap-1.5 btn-primary text-xs py-1.5 px-3">
                            {importing ? <Loader2 size={12} className="animate-spin" /> : <CheckCircle2 size={12} />}
                            {importing ? "Saving…" : `Save ${unimported}`}
                          </button>
                        )}
                        {importDone && pendingGeneration > 0 && (
                          <button onClick={handleGenerateAll} disabled={generatingAllImport} className="btn-primary text-xs py-1.5 px-3">
                            Generate all
                          </button>
                        )}
                      </div>
      ...
      {loadingSaved && !searching && <p className="text-ash text-xs mb-4">Loading previously found leads…</p>}
      {error && <p className="text-danger text-sm mb-4 whitespace-pre-wrap">{error}</p>}
      {searched && !error && results.length === 0 && (
        <p className="text-ash text-sm mb-4">No new leads found. Try a different niche, city, or country-wide search.</p>
      )}
                    ...
                    <span className="inline-flex items-center gap-2 flex-wrap">
                      {lead.demo_token ? (
                        <>
                          ...
                          <button onClick={() => handleGenerateEmail(lead)} disabled={generatingId === `email-${lead.id}`}
                      ...
                      <button onClick={() => handleDeleteLead(lead.id)} title="Remove from list — demo & sent emails kept"
                        className="text-ash/40 hover:text-danger transition-colors">
                        <Trash2 size={13} />
                      </button>
                    </span>
