...
export default function LeadFinder() {
  ...
  const [form, setForm] = useState({ niche:"", country:"Nigeria", city:"", state:"", max_leads:20 });
  ...
  useEffect(() => {
    api.get("/api/leads/niches")
      .then((res) => { setNiches(res.data); setForm((f) => ({ ...f, niche: res.data[0] })); })
      .catch(() => setError("Can't reach the backend right now."));
  }, []);
  ...
  // Add one manual entry to staging list
  ...
  // Push all staged leads to backend
      ...
      {results.length > 0 && (
        <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} className="card overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-ink-600">
            <span className="text-ash text-xs">{results.length} leads</span>
            {results.some((l) => !l.demo_token) && (
              <button
                onClick={async () => { for (const lead of results) { if (!lead.demo_token) await handleGenerate(lead); } }}
                disabled={!!generatingId}
                className="inline-flex items-center gap-1.5 text-xs text-ash-light hover:text-signal disabled:opacity-40">
            ...
            <tbody>
              {results.map((lead) => (
                <tr key={lead.id} className="border-t border-ink-600 hover:bg-ink-700/50">
                  <td className="px-4 py-3 text-ash-light font-medium">{lead.business_name}</td>
                  <td className="px-4 py-3 text-ash">{STATUS_LABELS[lead.website_status] || lead.website_status}</td>
                  <td className="px-4 py-3 text-ash font-mono text-xs">{lead.phone || lead.email || "—"}</td>
