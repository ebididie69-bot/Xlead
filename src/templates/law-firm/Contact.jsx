      ...
      <section className="px-8 py-16 max-w-6xl mx-auto grid md:grid-cols-2 gap-16">
        <Reveal>
          <p className="opacity-55 leading-relaxed mb-10">
            Get in touch with our team — we're happy to answer questions and arrange consultations.
          </p>
          {[
            { icon: Phone, label: "Phone", value: "Available on your listing" },
            { icon: MapPin, label: "Location", value: "Your address on file" },
        ...
        <Reveal delay={0.15}>
          <div className="p-10 flex flex-col gap-8" style={{ background: surface, borderTop: `2px solid ${accent}` }}>
            <div>
              <h2 className="text-2xl font-bold mb-3" style={{ fontFamily: "Georgia, serif" }}>Request a Consultation</h2>
              <p className="text-sm opacity-50 leading-relaxed">We'll be in touch within one business day.</p>
            </div>
            <motion.a href="#" whileHover={{ scale: 1.02, boxShadow: `0 0 30px ${accent}44` }} whileTap={{ scale: 0.98 }}
              className="flex items-center justify-center gap-2 py-4 font-bold"
            ...
            <div className="pt-6 border-t space-y-3" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
              <p className="text-xs opacity-30 uppercase tracking-widest">What to expect</p>
              <p className="text-sm opacity-50">Swift response and honest advice from our team.</p>
            </div>
          </div>
        </Reveal>
      </section>
