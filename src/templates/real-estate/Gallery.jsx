      ...
      <section className="px-8 pt-28 pb-16 max-w-6xl mx-auto border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3 mb-4"><div className="w-8 h-px" style={{ background: accent }} />
            <span className="text-xs font-bold tracking-[0.25em] uppercase" style={{ color: accent }}>Portfolio</span></div>
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight" style={{ fontFamily: "Georgia, serif" }}>Featured Properties</h1>
        </motion.div>
      </section>
          ...
          {slots.map(({ key, col, aspect }, i) => (
            <Reveal key={key} delay={i * 0.1} className={col}>
              <motion.div whileHover={{ scale: 1.01 }} className={`${aspect} overflow-hidden relative group`}>
                ...
                <div className="absolute bottom-0 left-0 w-0 group-hover:w-full h-0.5 transition-all duration-500" style={{ background: accent }} />
              </motion.div>
            </Reveal>
          ))}
        </div>
