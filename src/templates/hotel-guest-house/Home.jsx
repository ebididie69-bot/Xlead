...
export default function Home({ content, images, colors, base }) {
  ...
  const surface = colors.secondary || "#234D3C";
  ...
  return (
    <div style={{ background: bg, color: "#F5F0E8", minHeight: "100vh" }}>
      <section ref={heroRef} className="relative min-h-screen overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-8 pt-36 pb-24 min-h-screen flex flex-col justify-end">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}>
            <p className="text-xs font-bold tracking-[0.35em] uppercase mb-5" style={{ color: accent }}>Luxury Accommodation</p>
            <h1 className="text-5xl md:text-7xl font-bold leading-[0.95] tracking-tight mb-6 max-w-3xl" style={{ fontFamily: "Georgia, serif" }}>
              {content.hero_title || "A retreat worth returning to"}
            </h1>
            <p className="text-lg opacity-60 mb-10 max-w-lg leading-relaxed">{content.hero_subtitle || "Elegant rooms, attentive service, and an atmosphere of calm refinement."}</p>
            <motion.a href={`${base}/contact`} whileHover={{ scale: 1.03, boxShadow: `0 0 30px ${accent}44` }} whileTap={{ scale: 0.97 }}
              ...
              style={{ background: accent, color: "#1B3A2D" }}>
              {content.call_to_action?.button_text || "Reserve a Room"} <ChevronRight size={16} />
            </motion.a>
          </motion.div>
      ...
      <section className="py-20 px-8 max-w-6xl mx-auto border-t grid md:grid-cols-3 gap-10"
        style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          <Reveal key="✦ Complimentary Breakfast" delay={0.0}>
            <div className="flex gap-4">
              <div className="shrink-0 w-10 h-10 flex items-center justify-center text-lg" style={{ color: accent }}>◆</div>
              <div>
                <h3 className="font-bold mb-1">✦ Complimentary Breakfast</h3>
                <p className="text-sm opacity-50 leading-relaxed">Fresh seasonal breakfast included with every stay.</p>
              </div>
            </div>
          </Reveal>
          <Reveal key="✦ Free Wi-Fi" delay={0.1}>
            <div className="flex gap-4">
              <div className="shrink-0 w-10 h-10 flex items-center justify-center text-lg" style={{ color: accent }}>◆</div>
              <div>
                <h3 className="font-bold mb-1">✦ Free Wi-Fi</h3>
                <p className="text-sm opacity-50 leading-relaxed">High-speed internet throughout the property.</p>
              </div>
            </div>
          </Reveal>
          <Reveal key="✦ Concierge Service" delay={0.2}>
            <div className="flex gap-4">
              <div className="shrink-0 w-10 h-10 flex items-center justify-center text-lg" style={{ color: accent }}>◆</div>
              <div>
                <h3 className="font-bold mb-1">✦ Concierge Service</h3>
                <p className="text-sm opacity-50 leading-relaxed">Our team is available around the clock for your needs.</p>
              </div>
            </div>
          </Reveal>
      </section>
      ...
      <section className="py-24 text-center border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        <Reveal>
          <h2 className="text-3xl md:text-4xl font-bold mb-4" style={{ fontFamily: "Georgia, serif" }}>
            {content.call_to_action?.headline || "Reserve a Room"}
          </h2>
          ...
          <motion.a href={`${base}/contact`} whileHover={{ scale: 1.03, boxShadow: `0 0 30px ${accent}44` }}
            className="inline-flex items-center gap-2 px-10 py-4 font-bold" style={{ background: accent, color: "#1B3A2D" }}>
            {content.call_to_action?.button_text || "Reserve a Room"} <ChevronRight size={16} />
          </motion.a>
        </Reveal>
      </section>
