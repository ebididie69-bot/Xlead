...
export default function Home({ content, images, colors, base }) {
  ...
  const accent = colors.accent || "#FF6B00";
  const bg = colors.primary || "#1C1C1E";
  const surface = colors.secondary || "#2A2A2C";

  return (
    <div style={{ background: bg, color: "#F0EDE8", minHeight: "100vh" }}>
      <section ref={heroRef} className="relative min-h-screen overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-8 pt-36 pb-24 min-h-screen flex flex-col justify-end">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}>
            <p className="text-xs font-bold tracking-[0.35em] uppercase mb-5" style={{ color: accent }}>Master Builders</p>
            <h1 className="text-5xl md:text-7xl font-bold leading-[0.95] tracking-tight mb-6 max-w-3xl" >
              {content.hero_title || "Built to last. Delivered on time."}
            </h1>
            <p className="text-lg opacity-60 mb-10 max-w-lg leading-relaxed">{content.hero_subtitle || "Commercial and residential construction, from foundations to finish."}</p>
            <motion.a href={`${base}/contact`} whileHover={{ scale: 1.03, boxShadow: `0 0 30px ${accent}44` }} whileTap={{ scale: 0.97 }}
              ...
              {content.call_to_action?.button_text || "Get a Quote"} <ChevronRight size={16} />
            </motion.a>
          </motion.div>
      ...
      <section className="py-20 px-8 max-w-6xl mx-auto border-t grid md:grid-cols-3 gap-10"
        style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          <Reveal key="★ Fully Insured" delay={0.0}>
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-sm opacity-50 leading-relaxed">All works fully insured and certified to national standards.</p>
              </div>
            </div>
          </Reveal>
          <Reveal key="★ On Schedule" delay={0.1}>
            <div className="flex gap-4">
              <div className="shrink-0 w-10 h-10 flex items-center justify-center text-lg" style={{ color: accent }}>◆</div>
              <div>
                <h3 className="font-bold mb-1">★ On Schedule</h3>
                <p className="text-sm opacity-50 leading-relaxed">95% of projects delivered on or before the agreed deadline.</p>
              </div>
            </div>
          </Reveal>
          <Reveal key="★ Fixed Quotes" delay={0.2}>
            <div className="flex gap-4">
              <div className="shrink-0 w-10 h-10 flex items-center justify-center text-lg" style={{ color: accent }}>◆</div>
              <div>
                <h3 className="font-bold mb-1">★ Fixed Quotes</h3>
                <p className="text-sm opacity-50 leading-relaxed">Transparent pricing — the quote you get is the price you pay.</p>
              </div>
            </div>
          </Reveal>
      </section>
      ...
      <section className="py-24 text-center border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        <Reveal>
          <h2 className="text-3xl md:text-4xl font-bold mb-4" >
            {content.call_to_action?.headline || "Get a Quote"}
          </h2>
          ...
          <motion.a href={`${base}/contact`} whileHover={{ scale: 1.03, boxShadow: `0 0 30px ${accent}44` }}
            className="inline-flex items-center gap-2 px-10 py-4 font-bold" style={{ background: accent, color: "#1C1C1E" }}>
            {content.call_to_action?.button_text || "Get a Quote"} <ChevronRight size={16} />
          </motion.a>
        </Reveal>
      </section>
