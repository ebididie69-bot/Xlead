...
export default function Home({ content, images, colors, base }) {
  ...
  return (
    <div style={{ background: "#FFFFFF", color: "#0A0A0B", minHeight: "100vh" }}>
      <section className="relative min-h-[88vh] md:min-h-screen flex items-end md:items-center overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-6xl mx-auto px-5 sm:px-8 pb-16 md:pb-24 pt-28">
          <motion.h1
            ...
            className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight text-white max-w-xl"
          >
          ...
          <motion.p
            ...
            className="mt-4 text-white/70 max-w-md text-sm md:text-base leading-relaxed"
          >
            {content?.hero_subtitle ||
              "Stress-free rentals with simple booking, transparent rates, and a fleet you will love to drive."}
          </motion.p>
          ...
          <motion.div
            ...
            className="mt-8 rounded-2xl bg-white text-[#0A0A0B] p-3 md:p-4 shadow-xl max-w-3xl"
          >
            ...
            <a
              href={`${base}/contact`}
              className="mt-3 w-full md:w-auto md:float-right inline-flex justify-center px-8 py-2.5 rounded-lg text-sm font-semibold text-white"
            ...
            {BRANDS.map((b, i) => (
              <Reveal key={b} delay={i * 0.05}>
                <div className="rounded-2xl overflow-hidden relative aspect-[4/3] bg-neutral-100">
                  <SmartImage image={gallery[i] || images?.hero} accent={accent} className="w-full h-full absolute inset-0" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                  <p className="absolute bottom-3 left-3 text-white font-semibold text-sm z-10">{b}</p>
                </div>
              </Reveal>
            ))}
          </div>
            ...
            {CARS.map((c, i) => (
              <Reveal key={c.name} delay={i * 0.05}>
                <div className="rounded-2xl bg-white border border-black/5 p-4 shadow-sm h-full flex flex-col">
                  <div className="aspect-[16/10] rounded-xl overflow-hidden mb-3 bg-neutral-100 relative">
                    <SmartImage image={gallery[i] || images?.hero} accent={accent} className="w-full h-full absolute inset-0" />
                  </div>
                  <p className="font-semibold text-sm">{c.name}</p>
                  <p className="text-xs text-black/45 mt-1 flex-1">{c.price}</p>
                  <a href={`${base}/contact`} className="mt-3 text-center text-xs font-semibold py-2 rounded-lg border border-black/10">
                    Book Now
                  </a>
                </div>
              </Reveal>
      ...
      <section className="relative py-16 md:py-24 overflow-hidden min-h-[40vh]">
        <div className="absolute inset-0">
